...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
	...
	while (fgets(buf, BUF_SIZE, input) != NULL) {
		skip_comment(buf);
		char* pch = strtok(buf, IGNORE_CHARS);
		lnum++;
		...
		if (pch == NULL) {
			continue;
                ...
                /* Validate labels */
		int result = add_if_label(lnum, pch, addr, symtbl);

                /* This token is not a label */
		if (result == 0) {
                /* This token is a label but there is an error */
		} else if (result == -1) {
			err = -1;
			continue;
                /* This token is a right label */
		} else if (result == 1) {
		...
		while (pch != NULL) {
			args[num_args] = pch;
                        /* Validate labels */
			if (add_if_label(lnum, pch, addr, symtbl) == 1) {
				err = -1;
				continue;
			}
                        /* Validate # arguments */
			if (num_args >= MAX_ARGS) {
				raise_extra_arg_error(lnum, pch);
				err = -1;
				break;
			}
			num_args++;
		...
		int line = write_pass_one(output, name, args, num_args);
		addr = addr + line * 4;
	}
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
	...
	while (fgets(buf, BUF_SIZE, input) != NULL) {
		...
		lnum++;
                ...
                /* Next tokens are arguments for the instruction */
		while (pch != NULL) {
			args[num_args] = pch;
			pch = strtok(NULL, IGNORE_CHARS);
			num_args++;
		...
		if (translate_inst(output, name, args, num_args, addr, symtbl, reltbl) == -1) {
			err = -1;
			raise_inst_error(lnum, name, args, num_args);
		} else {
	...
	/* Since we pass this buffer to strtok(), the chars here will GET CLOBBERED.
	   ...
	   raise_inst_error(). 
	   Repeat until no more characters are left, and then return the correct return val */
}
